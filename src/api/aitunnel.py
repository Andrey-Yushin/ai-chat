...
class AITunnelClient:
    """
    Клиент для взаимодействия с AITunnel API.

    AITunnel - это сервис, предоставляющий унифицированный доступ к различным
    ...
    def __init__(self):
        ...
        if not self.api_key:
            ...
            self.logger.error("AITunnel API key not found in .env")
            # Выбрасывание исключения с понятным сообщением
            raise ValueError("AITunnel API key not found in .env")
        ...
        self.logger.info("AITunnelClient initialized successfully")
    ...
    def get_balance(self):
        ...
        try:
            ...
            response = requests.get(
                f"{self.base_url}/aitunnel/balance",  # Эндпоинт для проверки баланса
                headers=self.headers         # Заголовки с авторизацией
            )
