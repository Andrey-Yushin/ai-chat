...
Чат-приложение, использующее API AITUNNEL для взаимодействия с различными моделями искусственного интеллекта.
...
5. **Настройка переменных окружения**
   ...
   - Откройте `.env` и замените `your_api_key_here` на ваш API ключ AITUNNEL
   - Напишите `EMAIL_ADDRESS` который будет отправлять уведомления
   - Вставьте `EMAIL_PASS` пароль от приложения google. [Пароль можно создать по ссылке](https://myaccount.google.com/apppasswords)
   - Поставьте свой `EMAIL_RECIPIENT` на который будут приходить уведомления о балансе и уведомления об остатке бюджета на отправку запросов
...
│   ├── api/               # API интеграции
│   │   ├── __init__.py
│   │   └── aitunnel.py  # Взаимодействие с AITUNNEL API
...
1. **Чат с AI моделями**
   - Поддержка различных моделей через AITUNNEL API
